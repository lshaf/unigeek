...
Configure the GPIOs and bus options used by external modules (GPS, CC1101, NRF24, PN532, RFID/I2C devices) without rebuilding the firmware. Accessed from **Settings > Pin Setting** (identical to **Modules > Pin Setting**). Values are stored in `/unigeek/pin_config` on the active storage and reloaded at every boot.
